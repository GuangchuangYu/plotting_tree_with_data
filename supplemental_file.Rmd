...
The heat tree implemented in `metacoder` is designed for herachical data, but not for phylogeny. `metacoder` internally uses graph layout algorithms implemented in `igraph` [@igraph] and doesn't support widely used phylogenetic tree layouts (*e.g.* rectangular and slanted layouts). Edge length for phylogenetic tree is also not supported. 
